...
const char* password = "tu clave wifi";

// URL del servidor donde está el archivo api.php
const char* serverUrl = "https://alarmaapi-xt3odlrbrq-uc.a.run.app/alarmas";

// Identificador únic o del ESP32 (puede ser un nombre o código único)
const String espID = "esp4";
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpCode > 0) {
      if (httpCode == HTTP_CODE_OK) {
        ...
        StaticJsonDocument<2048> doc; // Ajusta el tamaño si la respuesta crece
        ...
        if (error) {
          ...
          for (JsonObject obj : doc.as<JsonArray>()) {
            ...
            if (id && espID == id) {
              ...
              if (estado && strcmp(estado, "activa") == 0) {
                Serial.println("Encendiendo el LED (estado: activa)...");
                if (LED_ACTIVE_HIGH) {
                  digitalWrite(ledPin, HIGH);
                } else {
                  digitalWrite(ledPin, LOW); 
              ...
              } else if (estado && strcmp(estado, "inactiva") == 0) {
                Serial.println("Apagando el LED (estado: inactiva)...");
                if (LED_ACTIVE_HIGH) {
                  digitalWrite(ledPin, LOW); 
                } else {
                  digitalWrite(ledPin, HIGH);
                ...
                Serial.print("Estado no reconocido para este dispositivo: ");
                Serial.println(estado ? estado : "NULL");
              }
              encontrado = true;
              break;
            }
